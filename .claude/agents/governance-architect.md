...
description: Specialist architect that extracts governance-layer patterns from a source AI Chief of Staff implementation — living constitution pattern, INDEX/MAP auto-regen scripts, audit cadence hooks, hard-limit enforcement, classification rules, authority hierarchy. Reads the live source implementation read-only, writes to Giovanni's `knowledge/`, `scripts/`, and `.claude/hooks/` paths.
