...
| [`/consistency-check`](consistency-check.md) | 8 semantic drift checks — memory + constitution + agent roster + decisions + decided-terms + knowledge↔knowledge + outbound published pages | `[--check <id>] [--since <YYYY-MM-DD>] [--write]` | `consistency-checker` | weekly (recommended) |
...
| [`/pulse`](pulse.md) | Weekly state-of-the-work read per work-block — code over tracker. Answers "where does each piece stand", not "what happened". | `[since=<YYYY-MM-DD>] [block=<slug>] [quick]` | main thread (+ agents per source) | weekly |
