...
What a node (a colleague's AI setup) must expose to be a valid Boss source.

The point of this contract is **normalization despite inconsistent quality**. Nodes will differ wildly
in how mature their knowledge/ústava/memory is. Boss does not demand they be good — it demands they
...
layout:                           # where Boss finds each layer (paths are node-relative)
...
**R1 — History is preserved.** Boss reads *into* the changelog and decision records to learn how &
...
```
[BOSS: public]      ← this line/section is shareable
[BOSS: restricted]  ← default; Boss in public-only mode will not read it
```

**R3 — Claims carry anchors.** A node's canonical/decision content should already tag provenance
(NEO does: `[ODHAD]`, `[DERIVED]`, primary anchors). Boss reuses whatever exists. Unanchored
claims are ingestable but capped at `singleton`/`likely` — they can never reach `canon-ready`.

## 3. What Boss reads, per layer
...
2. Make sure their repo history is intact and readable by Boss's reader.
3. Mark at least their shareable knowledge sections `[BOSS: public]`.
4. Add an entry to Boss's `sources/registry.yaml` (PR).
